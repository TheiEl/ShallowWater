...
subroutine solve(uout, vout, hout, uin, vin, hin)
  ...
  real*8, parameter :: dx20 = 1.0/dx, dy20 = 1.0/dy
  ...
  real*8, parameter :: nudxx = nu/(2.0*dx*dx), nudyy = nu/(2.0*dy*dy)

  ! Difference terms in the continuity and momentum eqzuations
  ...
  do j2 = 1, ny
    ...
    do i2 = 1, nx
      ...
      ! vyy diffusion term in x-momentum equation
      vyy = (vin(xc,yf) - 2.0*vin(xc,yc) + vin(xc,yb))*nudyy



      ! Summing up all the terms in the continuity equation
      hout(i2,j2) = -(hx + hy)

      ! Summing up all the terms in the x-momentum equation
      uout(i2,j2) = -(ux+uy) + fv - gx - bu + (uxx+uyy)

      ! Summing up all the terms in the y-momentum equation
      vout(i2,j2) = -(vx+vy) - fv - gy - bv + (vxx+vyy)
